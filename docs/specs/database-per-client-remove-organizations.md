# Database-per-client architecture: remove organizations

Status: Approved and implemented for fresh and single-client databases
...
The target application has no `organizations` table, no `organization_user` pivot, no `organization_id` columns, no `Organization` model, and no organization-scoping middleware or query clauses. Every database contains exactly one client's users and festival data. The database connection selected for the deployment is the tenant boundary.

This spec assumes **one application deployment/runtime is configured for one client database**. The deployment supplies the client identity and database credentials. The application does not select a tenant database from the request host at runtime.

That assumption is important. A shared runtime serving multiple client databases would require a trusted central client registry, host-to-client resolution before sessions/authentication, tenant-aware queue payloads, cache and filesystem scoping, and safe connection reset for long-lived workers. None of that infrastructure exists in this repository, and deleting `organizations` without adding it would make isolation unsafe.

## Goals

- Make the client database the only tenancy boundary.
...
- Make cross-client data access impossible through ordinary application queries because another client's rows are not present on the active connection.

## Non-goals

- Runtime host-based database switching in one Laravel process.
- A central control-plane database for client provisioning, billing, or support access.
- Cross-client reporting or global user accounts.
...
- Migrating cache entries, sessions, or queued jobs between clients.
...
3. **Client metadata** — `organizations.name` supplies the setup and application-shell name.
...
| `vendor_types` | `organization_id` | Remove column; all rows belong to the client database |
| `artist_types` | `organization_id` | Remove column; all rows belong to the client database |
...
- Each client has a dedicated database and a dedicated deployment configuration.
- All application tables, including `users`, live in that client database.
- Authentication is client-local. The same email may exist independently in multiple client databases.
- Laravel's default database connection remains the only application connection used by web requests and normal jobs.
- A deployment must have an immutable `CLIENT_KEY` for operational identification and a display `CLIENT_NAME`. Neither value is used to authorize database access; database credentials provide the boundary.
- `CLIENT_KEY` is used in logs, deployment inventory, backup names, and any shared Redis/filesystem prefix. It must not be derived from untrusted request input.
- `CLIENT_NAME` replaces `organizations.name` in shared UI props. `APP_NAME` remains the product name, `Artist Tree`.

### Client-level application state
...
Add a small service, for example `ClientContext`, as the single access point for:

- `name(): string` from `config('client.name')` / `CLIENT_NAME`.
...
- `User::effectiveEvent()` returns the user's current event when it still exists; otherwise it returns `ClientContext::defaultEvent()`.
- `User::setCurrentEvent(Event $event)` updates `users.current_event_id`.
- The initial setup event is set as both the client default and the completing user's current event.
- A newly provisioned or invited user with no current event inherits the client default.
- Deleting a selected event clears the foreign key; the next request falls back to the client default.
...
4. Make artist and artist-label names unique within the client database, which is equivalent to the existing organization-level intent.
...
Do not add `client_id`, `tenant_id`, or a renamed organization foreign key to tenant databases. That would retain row-scoped tenancy without providing isolation value.
...
- Refactor `User::effectiveEvent()` and `setCurrentEvent()` to use `users.current_event_id` and `ClientContext`.
...
- Add `ApplicationState` and the `ClientContext` service.
...
- Refactor `EnsureSetupComplete` and `PostLoginRedirect` to use `ClientContext::setupIsComplete()`.
...
- Keep authentication before all client data routes.

No middleware is needed to assert tenant membership: successful connection to the deployment's configured database establishes the tenant context, and every authenticated user in that database belongs to that client.

### Controllers and requests

- Delete `InteractsWithSetup::organization()`; retain or move only the timezone helper.
...
- Update the error copy from “this organization” to “this client” or “your festival.” Prefer user-facing “your festival”; “client” is an operational term.

### Inertia and Vue contracts

Replace the shared `organization` prop with:
...
- Setup controllers pass `client` rather than `organization`.
- Setup pages rename their prop and `:organization-name` usage to `client` / `clientName`.
- `SetupLayout` renders `CLIENT_NAME`; it must have a safe fallback such as `Festival setup`.
- `AppLayout` uses the current event name first and client name second, preserving the existing display order.
- Rename JavaScript variables such as `orgName` and `organizationItems` where they refer to the removed data model. The settings information architecture may still use the label “Organization Settings” if that remains the product term.
...
4. Seed one `application_state` row and a client-local admin user.
5. Configure `CLIENT_KEY`, `CLIENT_NAME`, and the client database credentials.
...
3. Decide how to handle users belonging to multiple organizations. Recommended: copy the user, including its password hash, into every applicable client database; the accounts become independent after cutover.
...
| --- | --- |
| `organizations.name` | Deployment `CLIENT_NAME` |
...
Preserve password hashes, verification timestamps, and user profile data. Clear `remember_token` and do not copy sessions so every user signs in against the correct client deployment after cutover.
...
Before routing traffic, record and compare per-client counts for users, events, locations, types, artists, labels, engagements, and label assignments. Also verify:
...
4. Start the client deployment and its queue workers.
5. Route only that client's hostname to the deployment.
6. Retain the shared source database read-only for the agreed rollback period.

Rollback means routing the client back to the unchanged shared application/database. Do not write back from a client database into the shared source. If client deployments have accepted writes, rollback requires a separate reconciliation plan.
...
2. configures `CLIENT_KEY`, `CLIENT_NAME`, and `DB_*`/`DB_URL`;
3. runs migrations;
4. ensures the singleton `application_state` row exists;
5. creates or imports the initial admin user;
6. runs a health check and a client-isolation smoke test.

Operational requirements:

- Run schema migrations against every client database before deploying code that requires the new schema. Track success per `CLIENT_KEY`.
- Back up and restore each database independently.
- Include `CLIENT_KEY` in structured logs and metrics, but never log database credentials.
- If Redis is shared, prefix cache, lock, session, and queue keys with the immutable client key. If filesystem/object storage is shared, prefix paths similarly.
- Ensure queue workers use the same client configuration as their web deployment. A job must never accept a client-provided database name or connection string.
...
- Incomplete client setup redirects authenticated users to setup.
- Completing setup is client-wide and applies to every user in that database.
- Setup creates the first event, makes it the client default, and selects it for the current user.
- A user-level current event overrides the client default.
- Clearing/deleting a user's selected event falls back to the client default.
- Event lists and settings query all events in the client database.
- Locked events remain readable and reject writes.
- Artist lists remain scoped to the user's current event.
- Artist histories and labels remain reusable across events in the same client database.
- Artist and label names remain case-insensitively unique under concurrent creation.
- Label and artist-type validation accepts only IDs present in the client database.
...
4. prove that an ID known only to client B returns 404/validation failure in client A because the row does not exist there.
...
- Add client configuration, `ApplicationState`, and `ClientContext`.
...
- Rewrite organization-based tests around client-local behavior.

### Phase 3: provisioning and migration tooling

- Add a repeatable client database provisioning workflow.
...
- Store migration audit output outside client databases.

### Phase 4: rehearsal and cutover

- Rehearse using a restored snapshot.
- Provision and verify every client deployment.
...
- The live client schema contains no `organizations` table, `organization_user` table, `organization_id` column, or `organization_artists` table.
- Runtime application code has no dependency on `App\Models\Organization` and no organization query scope or middleware.
- Every deployment connects to exactly one client database configured outside the request.
- Client display name, setup status, default event, and per-user current event all continue to work.
...
- Data-split verification proves complete per-client copies and zero rows from other clients.
...
- Backups, restore instructions, provisioning, migration orchestration, and rollback are documented for per-client databases.

## Decisions required before implementation

1. **Deployment topology:** approve the recommended one-deployment/one-database model. If a shared runtime must serve multiple client databases, this spec must be replaced with a runtime-tenancy design before code changes begin.
...
3. **Client name source:** approve `CLIENT_NAME` as deployment configuration rather than persistent editable data.
4. **Multi-organization users:** approve independent copied accounts in each client database.
